...
@NamedNativeQuery( //retrieves all undeleted LineItems in undeleted entries for an account when given an accountId
		...
		query = "SELECT SUM(parentInitialDebit) - SUM(parentInitialCredit) + SUM(childInitialDebit) - SUM(childInitialCredit) + SUM(debitAmount) - SUM(creditAmount) AS netAssets, :yearMonth AS yearMonth FROM    "
				+ "    (SELECT        "
