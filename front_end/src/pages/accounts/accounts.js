...
class Accounts extends React.Component {
    ...
    */
    ...
    renderAccountTypeSelect() {
        const accountTypeOptions = ACCOUNT_TYPE_OPTIONS(this.context.locale).filter(accountTypeOption => NON_CATEGORY_ACCOUNT_TYPES.includes(accountTypeOption.value))
        return(
            <Select
                options={accountTypeOptions}
                value={accountTypeOptions.find(accountTypeOption => accountTypeOption.value == this.props.match.params.activeTabId)}
                onChange={selectedOption => this.props.history.push(`/accounts/${selectedOption.value}`)}
            />
        )
    }
